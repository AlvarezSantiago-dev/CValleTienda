-- =============================================================
-- Movimientos de caja manuales: sesion_caja_id + RPCs
-- create / edit / delete atómicos (solo con caja abierta)
-- Excepción controlada al log inmutable: solo venta_id IS NULL
-- =============================================================

-- 1) Columna para atar el movimiento al turno
alter table public.movimientos_fondos
  add column if not exists sesion_caja_id uuid
    references public.sesiones_caja (id) on delete set null;

create index if not exists movimientos_fondos_sesion_idx
  on public.movimientos_fondos (sesion_caja_id)
  where sesion_caja_id is not null;

-- 2) Backfill best-effort: manuales con match único de sesión por tiempo
update public.movimientos_fondos mf
set sesion_caja_id = s.id
from public.sesiones_caja s
where mf.sesion_caja_id is null
  and mf.venta_id is null
  and mf.tienda_id = s.tienda_id
  and mf.created_at >= s.fecha_apertura
  and mf.created_at < coalesce(s.fecha_cierre, now() + interval '1 second')
  and (
    select count(*)::int
    from public.sesiones_caja s2
    where s2.tienda_id = mf.tienda_id
      and mf.created_at >= s2.fecha_apertura
      and mf.created_at < coalesce(s2.fecha_cierre, now() + interval '1 second')
  ) = 1;

-- -------------------------------------------------------------
-- Helper interno: efecto de un movimiento sobre el saldo
-- ingreso → +monto | egreso → -monto | ajuste → no usado en manuales
-- -------------------------------------------------------------
create or replace function public._delta_movimiento_fondo(p_tipo text, p_monto numeric)
returns numeric
language sql
immutable
as $$
  select case
    when p_tipo = 'ingreso' then p_monto
    when p_tipo = 'egreso' then -p_monto
    else 0
  end;
$$;

-- -------------------------------------------------------------
-- Registrar movimiento manual (caja abierta requerida)
-- -------------------------------------------------------------
...
comment on function public.registrar_movimiento_caja_manual is
  'Registra ingreso/egreso manual. Requiere sesión de caja abierta. Atómico con lock de cuenta.';

-- -------------------------------------------------------------
-- Editar movimiento manual (caja abierta + del turno actual)
-- -------------------------------------------------------------
...
comment on function public.editar_movimiento_caja_manual is
  'Edita un movimiento manual del turno abierto. Requiere caja abierta. Conserva usuario_id original.';

-- -------------------------------------------------------------
-- Eliminar movimiento manual (caja abierta + del turno actual)
-- -------------------------------------------------------------
...
comment on function public.eliminar_movimiento_caja_manual is
  'Elimina un movimiento manual del turno abierto y revierte el saldo. Requiere caja abierta.';

comment on column public.movimientos_fondos.sesion_caja_id is
  'Sesión de caja del turno. Seteada en manuales vía RPC; opcional en automáticos.';
...
grant execute on function public._delta_movimiento_fondo(text, numeric) to authenticated;

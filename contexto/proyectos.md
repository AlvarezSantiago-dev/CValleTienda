...
- **POS** — venta con búsqueda/scanner, multi-pago, cliente, descuentos. Cobro clásico: panel (cliente/descuento/notas) + modal de montos (`CobroPagoModal`). Modo guiado: wizard 4 pasos. En rubro `distribuidora`: Pedido (Contado / A cuenta), recargo %, remito automático y ledger `saldo_cc`. Tramos de cantidad (% o $ por presentación, desde N u.) en POS y catálogo.
...
- **Productos** — productos con variantes (ejes del rubro: Marca/Presentación, Talla/Color, …), códigos EAN-13, **foto de tapa + fotos por color** (Supabase Storage). Tramos de descuento por cantidad (**% o $**). **Packs N por producto** (x8, x24…) con foto y tramos por pack.
- **Catálogo público** — link `/c/{slug}`, productos marcados a mano, **destacados en coverflow**, grilla paginada (~20), chips de variante con labels del rubro, pedidos por WhatsApp, inbox `/pedidos` (editable en móvil). Distribuidora: aceptar emite remito; confirmar remito cobra y descuenta stock (un remito). Unidad o pack en la ficha. Plan: `planes/2026-08-25-catalogo-destacados-coverflow.md`.
...
- **Configuración** — datos de tienda; Cobros (`/configuracion/cobros`: cuentas de fondos + métodos en modal, orden auto); plantillas de etiqueta. Baja de cuenta (owner) en Avanzado; borrar cajero en Equipo.

...
describe('redondeo-efectivo', () => {
  ...
  it('renderAvisoRedondeoTicket usa default y reemplaza placeholders', () => {
    const t = renderAvisoRedondeoTicket(null, 40, 6460, '$')
    assert.match(t, /AVISO DE VUELTO/)
    assert.match(t, /\$ 40,00/)
    assert.match(t, /\$ 6\.460,00/)
  })

  it('renderAvisoRedondeoTicket respeta plantilla custom', () => {
    const t = renderAvisoRedondeoTicket(
      'Sin cambio chico: quedan {monto}. Compra {total}.',
      40,
      6460,
      '$'
    )
    assert.equal(t, 'Sin cambio chico: quedan $ 40,00. Compra $ 6.460,00.')
  })
})

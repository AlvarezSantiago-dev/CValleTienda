...
export async function obtenerSesionAbierta(): Promise<SesionConTotales | null> {
  ...
  const saldos: SaldoCuenta[] = ((cuentas ?? []) as Array<Record<string, unknown>>).map((c) => {
    const pos = posicionDeCuenta(
      {
        id: c.id as string,
    ...
    return {
      cuenta_fondo_id: pos.id,
      nombre: c.nombre as string,
      tipo: c.tipo as string,
      color: (c.color as string | null) ?? null,
      saldo_actual: pos.saldo_actual,
      saldoAlMomento: pos.saldoAlMomento,
      saldoProyectado: pos.saldoProyectado,
      porAcreditar: pos.porAcreditar,
      pendienteComision: pos.pendienteComision,
      proximaFechaAcreditacion: pos.proximaFechaAcreditacion,
      pendienteFechas: pos.pendienteFechas,
    }
  })

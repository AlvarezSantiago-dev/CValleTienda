...
export function PasoPago({
  ...
  const vuelto = Math.max(0, cobrado - totalAPagar)

  function seleccionar(metodoId: string) {
    if (totalAPagar <= 0) return
    onPagosChange(aplicarPagoRapido(metodoId, totalAPagar))
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-blue-50 rounded-xl p-4 text-center">
          <p className="text-sm text-blue-600 mb-1">Vuelto</p>
          <p className="text-lg font-bold text-blue-800 tabular-nums">{formatARS(vuelto)}</p>
        </div>
      </div>

      <p className="text-center text-sm text-gray-400">
        Tarjeta/MP: pago exacto · Efectivo: editá cuánto entrega el cliente
      </p>
    </div>
  )
}

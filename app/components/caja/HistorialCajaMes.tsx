  ...
  return (
    <section className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <h2 className="text-[15px] font-semibold text-[#0A0A0A]">Historial de sesiones</h2>
        <div className="flex items-center gap-1">
          <button
            onClick={() => mesAnterior && navegar(mesAnterior)}
            disabled={!mesAnterior}
            className="h-8 w-8 flex items-center justify-center rounded-full border border-gray-200 text-gray-600 hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
            title="Mes anterior"
          >
            ←
          </button>
          <span className="min-w-[140px] text-center text-sm font-medium text-[#0A0A0A] capitalize px-2">
            {mesLabel(mesActual)}
          </span>
          <button
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-gray-50">
                  <tr className="text-[10px] uppercase tracking-[0.08em] font-semibold text-gray-400 text-left">
                    ...
                    <th className="px-3 py-2 text-right">Diferencia ef.</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {sesiones.map((s) => (
                    <tr key={s.id} className="hover:bg-gray-50">
                      <td className="px-3 py-2 text-gray-900">{formatDateTime(s.fecha_apertura)}</td>
                      ...
                      <td className="px-3 py-2 text-right text-gray-900">
                        ...
                        <span className="text-gray-500">({formatARS(s.total_ventas_monto)})</span>
                      </td>
                      <td className="px-3 py-2 text-right">
                        {s.diferencia_efectivo == null ? (
                          ...
                          <span className="text-[#0A0A0A] font-semibold">+{formatARS(s.diferencia_efectivo)}</span>
                        ) : (
                      ...
                      <td className="px-3 py-2">
                        <Link href={`/caja/sesiones/${s.id}`} className="text-xs text-lime-700 hover:underline">
                          Ver →
                        </Link>
                      </td>
                    </tr>
  ...
  return (
    <div
      className={`rounded-xl border px-3 py-2.5 ${
        highlight ? 'border-lime-200 bg-lime-50' : 'border-gray-100 bg-white'
      }`}
    >
      <p className="text-[10px] uppercase tracking-[0.08em] font-semibold text-gray-400">{label}</p>
      <p className={`text-base font-semibold ${highlight ? 'text-lime-800' : 'text-gray-900'}`}>
        {value}
      </p>
    </div>
  )

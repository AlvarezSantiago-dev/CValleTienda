  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="bg-surface border border-border-subtle rounded-[var(--radius-lg)] p-5 space-y-4">
        <div>
          <p className="text-[10px] uppercase tracking-[0.10em] font-semibold text-fg-subtle mb-1">Tipo de negocio</p>
      ...
      <div className="bg-surface border border-border-subtle rounded-[var(--radius-lg)] p-5 space-y-3">
        <p className="text-[10px] uppercase tracking-[0.10em] font-semibold text-fg-subtle">Labels de variantes actuales</p>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <p className="text-xs text-fg-muted">Variante 1</p>
            <p className="text-sm font-medium text-fg">{CONFIG_RUBROS[selected].labelVar1}</p>
          </div>
          {CONFIG_RUBROS[selected].usarVar2 && (
            <div>
              <p className="text-xs text-fg-muted">Variante 2</p>
              <p className="text-sm font-medium text-fg">{CONFIG_RUBROS[selected].labelVar2}</p>
            </div>
          )}
        </div>
        <div>
          <p className="text-xs text-fg-muted mb-1">Unidades de medida disponibles</p>
          <div className="flex flex-wrap gap-1">
            {CONFIG_RUBROS[selected].unidadesDisponibles.map((u) => (
              <span key={u} className="rounded-[var(--radius-full)] bg-surface-sunken px-2 py-0.5 text-xs text-fg">
                {u}
              </span>
            ))}
          </div>

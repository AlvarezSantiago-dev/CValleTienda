...
| Tramos de cantidad | `producto_tramos_cantidad`: desde N u./packs → X %. Motor `lib/precios/tramos-cantidad.ts`. **Suma variantes** del mismo producto (misma presentación). Orden: lista → pack overlay → tramo del grupo → recargo CC. POS y catálogo. |
...
| Catálogo público | URL `/c/[slug]` (sin dashboard). Productos opt-in `visible_en_catalogo` (default off); **destacados** (`destacado_en_catalogo`, coverflow). Pedidos por WhatsApp. Inbox `/pedidos` + campana. Stock al confirmar envío/retiro. Grilla paginada + buscador/chips por URL. Stay-on-add + barra «Ver pedido». Spec: `referencia/catalogo-publico.md`. UOM pack/caja: stock en presentaciones; `unidades_contenido` es informativo (no multiplica el descuento). |

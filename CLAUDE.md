...
| Tramos de cantidad | `producto_tramos_cantidad` + `producto_pack_tramos`: desde N u./packs → **% o $** por presentación (no el total de la línea). Motor `lib/precios/tramos-cantidad.ts`. **Suma variantes** del mismo producto (misma presentación). Orden: lista → pack overlay → tramo del grupo → recargo CC. POS y catálogo. |
...
| Pedidos catálogo | Editables (qty/ítems, tope de stock) hasta cobrar. Distribuidora + plan remitos: **aceptar emite remito** (sin stock ni cobro); **confirmar remito** = cobro (seña CC o métodos) + stock + venta, reusa ese remito. Sin remitos: aceptar = estado; stock al cobrar. |
...
| Catálogo público | URL `/c/[slug]` (sin dashboard). Productos opt-in `visible_en_catalogo` (default off); **destacados** (`destacado_en_catalogo`, coverflow). Pedidos por WhatsApp. Inbox `/pedidos` + campana. Stock al **confirmar remito** (o al cobrar si el rubro no usa remitos). Grilla paginada + buscador/chips por URL. Stay-on-add + barra «Ver pedido». Spec: `referencia/catalogo-publico.md`. Chips de variante = labels del rubro (distribuidora: Marca / Presentación). UOM pack/caja: stock en presentaciones; `unidades_contenido` es informativo (no multiplica el descuento). |
...
| Rendimiento escala | `requireAuthCtx` + `createClient` cacheados; layout sin leer ventas del turno; RLS wrap + `pg_trgm`; catálogo público con `unstable_cache`/`revalidateTag`; listados sin embed masivo de variantes. Guía: `referencia/rendimiento-escala.md`. Migraciones `20260829000002`, `20260829000003`. |
